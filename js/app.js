(() => {
            ...
            if (tabsContent.length > 0) {
                ...
                tabsContent.forEach(((tabsContentItem, index) => {
                    if (tabsTitles[index].classList.contains("_tab-active")) {
                        if (tabsBlockAnimate && document.documentElement.clientWidth < tabsBlock.getAttribute("data-tabs")) _slideDown(tabsContentItem, tabsBlockAnimate); else tabsContentItem.hidden = false;
                        if (isHash && !tabsContentItem.closest(".popup")) setHash(`tab-${tabsBlockIndex}-${index}`);
                    } else if (tabsBlockAnimate && document.documentElement.clientWidth < tabsBlock.getAttribute("data-tabs")) _slideUp(tabsContentItem, tabsBlockAnimate); else tabsContentItem.hidden = true;
                }));
            }
        }
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) new core(".swiper", {
            ...
            navigation: {
                prevEl: ".slider-searching__prev",
                nextEl: ".slider-searching__next"
            },
            on: {}
        });
    }
    ...
    const headerHidden = () => {
        ...
        const disappearanceHeader = function(entries, observer) {
            const entry = entries[0];
            if (entry.isIntersecting) header.style.opacity = "0"; else header.style.opacity = "1";
        };
